...
import numpy as np
from enhancersdata import SeqDataSet
...
def run_experiment(general_cfg, seed):
    ...
    np.random.seed(seed)
    ds = SeqDataSet('/cs/grad/pazbu/paz/dev/projects/dnanet-v2/data')
    ...
    iters_per_epoch = int(ds.num_examples / mini_batch_size)
        ...
        for run_idx in range(num_runs):
            ...
            for epoch_idx in range(num_epochs):
                for iter_idx in range(iters_per_epoch):
                    batch = ds.next_batch(mini_batch_size)
                    if iter_idx % 100 == 0:
                        # out = y_.eval(feed_dict={x: batch[0], y_: batch[1], keep_prob: 1.0})
                        # print(out)
                        train_accuracy = accuracy.eval(feed_dict={x: batch[0], y_: batch[1], keep_prob: 1.0})
                        # valid_accuracy = accuracy.eval(feed_dict={x: mnist.validation.images,
                        #                                           y_: mnist.validation.labels,
                        #                                           keep_prob: 1.0})
                        # print('%d, %d, %d, %g, %g' %
                        #       (run_idx, epoch_idx, iter_idx, train_accuracy, valid_accuracy))
                        print('%d, %d, %d, %g, %g' %
                              (run_idx, epoch_idx, iter_idx, train_accuracy, 0))
