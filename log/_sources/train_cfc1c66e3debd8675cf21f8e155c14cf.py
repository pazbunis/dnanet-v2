...
def CNN(x, general_cfg, conv1_cfg, conv2_cfg, conv3_cfg, pool1_cfg, pool2_cfg, pool3_cfg, dropout_keep_prob):
    ...
    bn2 = tf.layers.batch_normalization(max_pool2)
    conv3 = tf.layers.conv2d(inputs=bn2, filters=conv3_cfg["num_filters"], kernel_size=conv3_cfg["filter_size"],
                             activation=tf.nn.relu,
    ...
    # flatten
    bn3 = tf.layers.batch_normalization(max_pool3)
    conv3_flat = tf.contrib.layers.flatten(bn3)
